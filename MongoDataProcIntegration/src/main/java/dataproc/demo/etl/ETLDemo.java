...
public class ETLDemo {
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
		      ...
		      System.err.println("Usage: JavaWordCount <file> " + args[0]);
		      System.err.println("Usage: JavaWordCount <file> " + args[1]);
		      System.exit(1);
		}
		String mongouri = args[0];
		String gcpbucket = args[1];
		
		SparkSession spark = SparkSession.builder()
			      //.master("local")
			      .appName("MongoSparkConnectorIntro")
			      //.config("spark.mongodb.output.uri", "mongodb+srv://main_user:pare1212@gcpdemocluster-fhbg9.gcp.mongodb.net/test.myCollection")
			      //.config("spark.mongodb.input.uri", "mongodb+srv://main_user:pare1212@gcpdemocluster-fhbg9.gcp.mongodb.net/test.myCollection")
			      .getOrCreate();
		JavaSparkContext sc = new JavaSparkContext(spark.sparkContext());
		//gs://paresh_bucket/relationalData/fin_district.tsv
		String path = gcpbucket+"/fin_district.tsv";
	    ...
	    writeOverrides1.put("uri", mongouri);
         ...
         * Client collection
         */
		JavaRDD<String> clientLines = sc.textFile(gcpbucket+"/fin_client.tsv");
		JavaRDD<String> dispLines = sc.textFile(gcpbucket+"/fin_disp.tsv");
		JavaRDD<String> cardLines = sc.textFile(gcpbucket+"/fin_card.tsv");
         ...
         * Account collection
         */
		JavaRDD<String> accLines = sc.textFile(gcpbucket+"/fin_account.tsv");
		JavaRDD<String> loanLines = sc.textFile(gcpbucket+"/fin_loan.tsv");
		JavaRDD<String> orderLines = sc.textFile(gcpbucket+"/fin_order.tsv");
		JavaRDD<String> transLines = sc.textFile(gcpbucket+"/fin_trans.tsv");
        ...
        JavaPairRDD<Integer, Iterable<Transaction>> transactions = transLines.mapToPair(x -> getTrans(x)).
        		                                                       groupByKey();
        
        JavaPairRDD<Integer, Tuple2<Account, Optional<Iterable<Loan>>>> accountLoans = 
        		                                    accounts.leftOuterJoin(loans);
        JavaPairRDD<Integer, Account> loansToAccountaccount = accountLoans.mapToPair(x -> loansToAccount(x));        
        
        JavaPairRDD<Integer, Tuple2<Account, Optional<Iterable<Order>>>> accountOrders =  
        		                                    loansToAccountaccount.leftOuterJoin(orders);
        JavaPairRDD<Integer, Account> ordersToAccount = accountOrders.mapToPair(x -> ordersToAccount(x));
        
        JavaPairRDD<Integer, Tuple2<Account, Optional<Iterable<Transaction>>>> accountTrans = 
        		                                    ordersToAccount.leftOuterJoin(transactions);
	    ...
	    writeOverrides.put("uri", mongouri);
